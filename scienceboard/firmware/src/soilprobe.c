...
void soilprobe_cmd(struct soilprobe_cmd *cmd, struct soilprobe_resp *resp)
{
	...
	strncpy(&(cmdbuf[CMD_ADDR_OFF]), cmd->addr, 3);

	/* Copy command string, args, and CRLF */
	if (cmd->cmd == CMD_GET_READING)  /* Get Reading */
	{
		cmdbuf[CMD_CMD_OFF] = cmdstr[0];
		cmdbuf[CMD_CMD_OFF + 1] = cmd->arg[0];
		memcpy(&(cmdbuf[CMD_CMD_OFF + 2]), "\x0D\x0A", 2);
	}
	...
	{
		memcpy(&(cmdbuf[CMD_CMD_OFF]), cmdstr, 3);
		memcpy(&(cmdbuf[CMD_CMD_OFF + 3]), cmd->arg, cmd->argsize);
		memcpy(&(cmdbuf[CMD_CMD_OFF + 3 + cmd->argsize]), "\x0D\x0A", 2);
	}
	else  /* Get commands */
	{
		memcpy(&(cmdbuf[CMD_CMD_OFF]), cmdstr, 4);
		memcpy(&(cmdbuf[CMD_CMD_OFF + 4]), "\x0D\x0A", 2);
	}
